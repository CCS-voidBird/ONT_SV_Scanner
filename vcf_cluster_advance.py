...
CALLERS=["cuteSV","nanoSV","sniffles","svim","noname"]
MAPPERS=["minimap2","ngmlr","globalMapping"]
CHROMOSOME=[str(x) for x in range(1,30)]+["X","MT"]
#CHROMOSOME=["BrahChr{}_1".format(x) for x in CHROMOSOME]
LABELS={
    ...
    "noname":["CHROM","POS","REF","variants/ALT","ID","END","SVTYPE","SVLEN","SUPPORT"]
}
...
ALTS={
    ...
    "noname":{"DEL":"DEL","INS":"INS","INV":"UNO","DUP":"DUP","DUP:TANDEM":"DUP","DUP:INT":"DUP","BND":"BND","INVDUP":"DUP"}
}
DEFAULT=["DEL","INS","DUP"]
...
class Caster:
    ...
    def scan(self,locat,threshold=2, mode="normal"):
        """
        ...
        if mode == "normal":
            ...
            for key in ["minimap2","ngmlr"]:
                mapper_stat[key] = []
        if mode == "global":
            for key in ["globalMapping"]:
                mapper_stat[key] = []
            for key in self.combos:
                caller_stat[key.replace("_globalMapping",'')] = []
            print(caller_stat)
            print(mapper_stat)
            print("Global mode.")
        ...
        if self.subvcf != None:
            for chr in CHROMOSOME:
                ...
                if len(X) != 0:
                    ...
                    groups = set([x for x in o_pred if x != -1])
                    ...
                    for group in groups:
                        chr_group = chr+"_"+self.sv+"_"+str(group)
                        res = self.vcf[chr][self.vcf[chr]["group2"]==group]["caller"].tolist()
                        if len(set(res)) > 1:
                            results.append(self.vcf[chr][self.vcf[chr]["group2"]==group])
                            backups.append(backup_record[backup_record["group2"]==chr_group])
                        else:
                            single_group = backup_record[backup_record["group2"]==chr_group]
                            single_group["group2"] = single_group["combo"]
                            backups.append(single_group)
            ...
            for combo in self.combos:
                if mode == "normal":
                    ...
                    key_caller = combo.replace("_globalMapping",'')
                    key_mapper = "globalMapping"
            ...
            plot_venn(mapper_stat, locat, self.sv,mode=mode)
            print("Making caller venn.")
            plot_venn(caller_stat, locat, self.sv,mode=mode)
...
def plot_venn(data,locat,sv,mode="normal"):
    ...
    if mode == "normal":
        if len(data.keys()) == 2:
            ...
            name = "caller"
            BioVenn.draw_venn(tuple(data["cuteSV"]), tuple(data["sniffles"]), tuple(data["svim"]), subtitle="",
                              nrtype="pct",
                              xtitle="cuteSV", ytitle="sniffles", ztitle="svim",
                              title="{} Overlap of Callers".format(sv),
                              output="png", filename="{}/{}_{}_venn.png".format(locat, name, sv))
    ...
    elif mode == "global" and len(data) >= 2:
        print("ploting")
        samples = list(data)
        print(samples)
        new_data = {}
        ...
        name = "caller"
        """
        BioVenn.draw_venn(tuple(data[samples[0]]), tuple(data[samples[1]]), tuple(samples[2]), subtitle="",
                          nrtype="pct",
                          xtitle=samples[0], ytitle=samples[1], ztitle=samples[2],
                          title="{} Overlap of samples".format(sv),
                          output="png", filename="{}/{}_{}_venn.png".format(locat, name, sv))
        """
        fig = venn.venn(new_data, fmt="{percentage:.2f}%")
        plt.title("Overlap within Callers")
...
def vcfs_eater(file_names,threshold,mode="normal"):
    """
    ...
    if mode == "normal":
        ...
        for path, caller, mapper in file_names:
            print(path)
            df = allel.vcf_to_dataframe(path, fields=["CHROM","POS","REF","variants/ALT","ID","END","SVTYPE","SVLEN","SUPPORT"])
            #print("reading geno for {}".format(path))
            #geno = [x[0] for x in allel.read_vcf(path, fields=["GT"])["calldata/GT"].tolist()]
            #df["GT"] = geno
            df = vcf_adjuster(df, "noname", mapper, threshold)
            print("now adjust {}".format(path))
            cleandata = VcfRecord(caller, mapper, df)
            samplename = path.split("/")[-1].split(".")[0]
            df["caller"] = samplename
            df["combo"] = "{}_{}".format(caller, mapper)
            cleandata.name = samplename
            record["{}_{}".format(caller, mapper)] = cleandata
            vcf_combo.append("{}_{}".format(caller, mapper))
...
def main():
    ...
    req_grp.add_argument('-m', '--mode', type=str, help="clustering mode", default="normal")
    file_names = []
    ...
    record,vcf_combo = vcfs_eater(file_names,threshold,mode=mode)
    ...
    for sv in DEFAULT:
        ...
        castbook.append(casters[sv].scan(locat,threshold=2,mode=mode))
