...
INTRO = "SV Scanner -- vcf_inheritance_cluster was designed for detecting inherited SVs from given family pairs.\nCurrently support vcf_cluster outputs and these Aligners: Minimap2, Ngmlr; SV caller: cuteSV, SVIM, Sniffles, NanoSV."
...
def vcf_get_loc(vcf):
    """
    get SVs' center position and 1/2 length
    """
    try:
...
class Caster:
    """
    main class for clustering and inheritance test.
    """
    ...
    def cast(self,size=0,eps=10):
        ...
        :param size: a threshold for SV length
        :param eps: searching range for DBSCAN
        :return: df for all valid SVs
        """
        ...
        match_records = []
        valid = []  # Inherited SVs
        for chr in CHROMOSOME: # get chromosome
            for sv in self.svs: # get SV type
                true_shoot = 0
                false_shoot = 0
                miss_shoot = 0
                true_call = 0
                ...
                if len(pts) != 0:
                    ...
                    mref = mother[gtmask] # homo groups in mother
                    gtmask = off["GT"].apply(lambda x: x == [1,1])
                    oref = off[gtmask] # homo groups in child
                    ...
                    for group in mgroups:
                        ...
                        if [1, 1] in gts and [0,0] not in gts:
                            ...
                            if len(suboff) == 0:
                                ...
                                valid.append(off[off["group"] == group])
                                valid.append(mref[mref["group"] == group])
                                true_shoot += 1
                    ...
                    for group in ogroups:
                        ...
                        if [1, 1] in gts and [0,0] not in gts:
                            ...
                            if len(submother) == 0:
                                ...
                                valid.append(oref[oref["group"] == group])
                                valid.append(mother[mother["group"] == group])
                                true_shoot += 1
        ...
        self.valid = pd.concat(valid,axis=0)
        all_gt = self.valid["GT"]
        all_gt = ["/".join([str(x[0]),str(x[1])]) for x in all_gt] # format gt data to a readable version
        self.valid["GT"] = all_gt
        self.valid.drop_duplicates(inplace=True)
        ...
        all_record = all_true_shoot+all_false_shoot+all_miss_shoot

        """
        merge inheritance results to a overall list
        """
        for sv in self.svs:
        ...
        output.append(["all",all_true_shoot,all_false_shoot,all_record,presice,all_miss_shoot,all_true_call,all_false_call])
    ...
    def output(self,locat,sample):
        ...
        """
        allel.write_vcf("{}/{}/vcf/{}_valid.vcf".format(locat,sample,self.combo),self.valid,description=NOTE)
...
def main():
    ...
    if len(mother_combos) != len(offspring_combos):
        ...
        for combo in mother_combos:
            ...
            overlap = caster.output(locat,sample)
